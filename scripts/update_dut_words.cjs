...
const dutWordData = {
    'duty': { prefix: '', prefixMeaning: '', prefixMeaningEn: '', root: 'duty', rootMeaning: '职责', rootMeaningEn: 'duty', rootOrigin: '古法语', rootOriginNote: '源自古法语devoir', suffix: '', suffixMeaning: '', suffixMeaningEn: '' }
};

let updated = 0;
words.forEach(w => {
    const wordLower = w.word.toLowerCase();
    if (dutWordData[wordLower]) {
        const d = dutWordData[wordLower];
        w.prefix = d.prefix;
...
console.log('Updated ' + updated + ' DUT words!');
