...
if (match) {
  ...
  words.forEach((w, idx) => {
    ...
    const originalWord = w.word.toLowerCase();
    ...
    // 处理 "XXX n. XXX" 或 "XXX v. XXX" 格式 - 去掉后面的词性和含义
    // 匹配 空格+词性.+中文 的模式
    meaning = meaning.replace(/\s+(n\.|v\.|adj\.|adv\.)\s*/g, '；');
    ...
    // 如果meaning为空、太短、或者只包含原单词（说明没有正确翻译），使用单词本身
    if (!meaning || meaning.length < 1 || meaning.toLowerCase() === originalWord) {
      meaning = w.word;
    }
