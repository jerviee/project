...
const degWordData = {
    'degree': { prefix: '', prefixMeaning: '', prefixMeaningEn: '', root: 'degr', rootMeaning: '步、等级', rootMeaningEn: 'step, grade', rootOrigin: '拉丁语', rootOriginNote: '源自拉丁语gradus', suffix: '-ee', suffixMeaning: '名词后缀', suffixMeaningEn: 'noun suffix' }
};

let updated = 0;
words.forEach(w => {
    const wordLower = w.word.toLowerCase();
    if (degWordData[wordLower]) {
        const d = degWordData[wordLower];
        w.prefix = d.prefix;
...
console.log('Updated ' + updated + ' DEG words!');
