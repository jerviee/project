...
const cyWordData = {
    'cycle': { prefix: '', prefixMeaning: '', prefixMeaningEn: '', root: 'cycl', rootMeaning: '圆、环', rootMeaningEn: 'circle, ring', rootOrigin: '希腊语', rootOriginNote: '源自希腊语kyklos', suffix: '-e', suffixMeaning: '名词后缀', suffixMeaningEn: 'noun suffix' }
};

let updated = 0;
words.forEach(w => {
    const wordLower = w.word.toLowerCase();
    if (cyWordData[wordLower]) {
        const d = cyWordData[wordLower];
        w.prefix = d.prefix;
...
console.log('Updated ' + updated + ' CY words!');
