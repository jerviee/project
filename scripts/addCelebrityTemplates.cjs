...
function tsStringify(obj, indent = 0) {
  ...
  if (Array.isArray(obj)) {
    ...
    const items = obj.map(item => tsStringify(item, indent + 1)).join(',\n');
    return `[\n${items}\n${spaces}]`;
  }
  ...
  if (typeof obj === 'object') {
    ...
    const entries = keys.map(key => {
      const value = tsStringify(obj[key], indent + 1);
...
const templatesStr = celebrityTemplates.map(t => tsStringify(t, 2)).join(',\n');

lines.splice(arrayEndIndex, 0, templatesStr);

fs.writeFileSync('./src/data/writing.ts', lines.join('\n'), 'utf8');
console.log(`Added ${celebrityTemplates.length} celebrity essay templates successfully!`);
