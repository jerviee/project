...
const derWordData = {
    'derive': { prefix: '', prefixMeaning: '', prefixMeaningEn: '', root: 'deriv', rootMeaning: '获得', rootMeaningEn: 'to derive', rootOrigin: '拉丁语', rootOriginNote: '源自拉丁语derivare', suffix: '-e', suffixMeaning: '动词后缀', suffixMeaningEn: 'verb suffix' }
};

let updated = 0;
words.forEach(w => {
    const wordLower = w.word.toLowerCase();
    if (derWordData[wordLower]) {
        const d = derWordData[wordLower];
        w.prefix = d.prefix;
...
console.log('Updated ' + updated + ' DER words!');
