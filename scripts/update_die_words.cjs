...
const dieWordData = {
    'die': { prefix: '', prefixMeaning: '', prefixMeaningEn: '', root: 'die', rootMeaning: '死亡', rootMeaningEn: 'to die', rootOrigin: '古英语', rootOriginNote: '源自古英语deagian', suffix: '', suffixMeaning: '', suffixMeaningEn: '' }
};

let updated = 0;
words.forEach(w => {
    const wordLower = w.word.toLowerCase();
    if (dieWordData[wordLower]) {
        const d = dieWordData[wordLower];
        w.prefix = d.prefix;
...
console.log('Updated ' + updated + ' DIE words!');
